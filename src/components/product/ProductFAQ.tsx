...
const ProductFAQ = ({
  ...
  return (
    <div className="w-full flex flex-col gap-12 py-16 px-6 md:px-16 bg-[#F8F7FD] rounded-2xl">
      <div className="flex flex-col items-center w-full gap-3">
        ...
        <p className="text-center text-[#666] max-w-xl mt-4">
          Everything you need to know about our digital photo frame
        </p>
      </div>
        ...
        <Accordion type="single" collapsible className="w-full">
          {items.map((item, index) => (
            <AccordionItem 
              key={`faq-${index}`} 
              value={`item-${index + 1}`} 
              className={`${index === 0 ? 'border-t ' : ''}border-b border-[#EAEAEA] overflow-hidden`}
            >
