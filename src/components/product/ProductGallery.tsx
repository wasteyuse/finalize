  ...
  return <div className="mb-8">
      ...
      <div className="relative w-full h-[329px] mb-4 overflow-hidden rounded-xl shadow-md">
        ...
        <div className="absolute inset-0 flex items-center justify-between px-3 opacity-0 hover:opacity-100 transition-opacity duration-200">
          ...
          <button className="w-9 h-9 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-full shadow-md hover:bg-white transition-colors" onClick={() => setCurrentImage(prev => prev < images.length - 1 ? prev + 1 : 0)} aria-label="Next image">
            <ChevronRight className="w-5 h-5 text-gray-700" />
          </button>
        </div>
        
        {/* Zoom button */}
        <button className="absolute w-9 h-9 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-full shadow-md hover:bg-white transition-colors right-3 bottom-3" aria-label="Zoom image" onClick={() => setIsZoomed(!isZoomed)}>
